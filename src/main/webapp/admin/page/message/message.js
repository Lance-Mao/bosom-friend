...
layui.config({
    ...
    //加载数据
    function loadPage(leaveMessage) {
        $.get("../../json/message.json", function (data) {
            ...
            for (var i = 0; i < leaveMessage.length; i++) {
                ...
                msgHtml += '    <img src="' + baseUrl+"/images/user"+leaveMessage[i].userImg + '" width="50" height="50"><input type="hidden" value="' + data[i].msgId + '">';
                msgHtml += '    <div class="user_info">';
                msgHtml += '        <h2>' + leaveMessage[i].user_id + '</h2>';
                msgHtml += '        <p>' + leaveMessage[i].content + '</p>';
                msgHtml += '    </div>';
                msgHtml += '  </td>';
                msgHtml += '  <td class="msg_time">' + leaveMessage[i].date + '</td>';
                    ...
                    $.get("../../json/message.json", function (data) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            if (data[i].msgId == id) {
                                ...
                                msgReplyHtml += '    <div class="user_info">';
                                msgReplyHtml += '        <h2>' + leaveMessage[i].user_id + '</h2>';
                                msgReplyHtml += '        <p>' + leaveMessage[i].content + '</p>';
                                msgReplyHtml += '    </div>';
                                msgReplyHtml += '  </td>';
        ...
        $(".send_msg").click(function () {
            if (layedit.getContent(editIndex) != '') {
                ...
                replyHtml += '    <img src="' + baseUrl+"/images/user"+leaveMessage[i].userImg + '" width="50" height="50">';
                replyHtml += '    <div class="user_info">';
                replyHtml += '        <h2>'+leaveMessage[i].user_id+'</h2>';
